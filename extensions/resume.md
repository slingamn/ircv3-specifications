...
Upon establishing the new connection, the client begins capability negotiation, negotiates all mutually-supported capabilities, and MUST confirm that the `draft/resume-0.5` capability exists. If this capability does not exist, the client continues connection registration without attempting to resume. If this capability does exist, the client sends the `RESUME` command and MUST wait for either a `RESUME` or a `FAIL RESUME` message from the server before continuing registration. It should be noted that the client SHOULD NOT perform SASL authentication if the `draft/resume-0.5` capability exists and they wish to resume their session (and have not yet received a `FAIL RESUME` message), since a successful resumption will restore their logged-in status, making SASL redundant.
